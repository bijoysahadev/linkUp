...
const MakeProfile = ({mainClassname,profileImage,profileName,profilStatus,onclick,buttonText,type,buttonTextTwo,onclicktwo}) => {
  return (
   <div className={`flex items-center justify-between border-b border-black/30 pb-2 ${mainClassname} `} >
    ...
    <p  className='text-xs text-[#4D4D4D] font-normal' >{profilStatus}</p>
     ...
     {
      type=="secondbtnneed"  &&  <button onClick={onclicktwo} className='text-sm text-white px-5 font-base py-0.5 rounded-md bg-[#5F35F5]' >{buttonTextTwo}</button>
     }
